...
**FR-006**: 📋 PLANNED - Forgot Password / Password Reset: `POST /auth/forgot-password` (email → time-limited token) + `POST /auth/reset-password` (token + new password). Two new frontend pages. Schema: `password_reset_token` + `password_reset_expires` on User. Login page link already exists pointing to `/auth/forgot-password` (currently 404s). Docs: docs/feature-requests/FR-006-forgot-password.md
**FR-007**: 📋 PLANNED - Order Confirmation Emails: transactional email sent to customer after Stripe webhook `checkout.session.completed` and PayPal `captureOrder()`. Separate `OrderEmailService`. Content adapts by order type (digital/physical/service). No frontend changes. Docs: docs/feature-requests/FR-007-order-confirmation-emails.md
