...
```
domain        String @unique  — the secondary domain (e.g., secondary.com)
...
The simplest possible implementation. When a request arrives at the application from `secondary.com`, redirect the browser to `yourdomain.com/author`.
...
export async function middleware(request: NextRequest) {
  ...
  if (host && host !== baseDomain && !host.includes('localhost')) {
    ...
    if (alias) {
      ...
      // e.g., secondary.com/bio → yourdomain.com/author/bio
...
**DNS requirement**: For a request to arrive at your server from `secondary.com`, the domain owner must have a DNS A/CNAME record pointing `secondary.com` to your server's IP/hostname. This is separate from the verification TXT record already in the system. The backstage UI should explain both:
...
This is the more ambitious feature: a visitor goes to `secondary.com/bio` and sees the content from `yourdomain.com/author/bio`, but their browser address bar still shows `secondary.com/bio`. This is a **reverse proxy with path rewriting**.

### Feasibility Analysis

**The core challenge**: When a browser is at `secondary.com`, same-origin policy means all requests (API calls, asset fetches, navigation) must also go to `secondary.com`. If the application is only deployed at `yourdomain.com`, any `<Link>` the user clicks would take them to `yourdomain.com`. To maintain the `secondary.com` URL persistently, every subsequent navigation must also be intercepted.
...
Next.js middleware runs at the edge and can rewrite requests internally without a redirect. The browser never sees the internal rewrite — it only sees the `secondary.com` URL.

```typescript
// middleware.ts
if (alias && alias.alias_type === 'proxy') {
  // Rewrite: serve /author/bio content when request comes in as secondary.com/bio
...
**How it works**: `NextResponse.rewrite()` changes what URL the Next.js router resolves on the server, but the browser never sees it. The HTML is served with `secondary.com` as the base. Subsequent `<Link>` navigations will also hit `secondary.com` and be intercepted and rewritten by middleware again.

**Prerequisites**:
- The DNS A/CNAME for `secondary.com` must point to the same server as `yourdomain.com`
...
- Absolute links in page content (e.g., hardcoded `href="https://yourdomain.com/shop"`) will break the alias — the visitor will leave `secondary.com`. The page builder should generate relative links.
...
- SEO: `<link rel="canonical">` should point to `yourdomain.com/author` even when browsing at `secondary.com/bio`. This prevents duplicate content indexing.
...
1. Accepts requests on `secondary.com`
2. Forwards them to `yourdomain.com/author{path}` 
3. Rewrites the response HTML to replace `yourdomain.com` with `secondary.com` in absolute URLs and canonical tags
...
If both domains are behind Cloudflare, a Cloudflare Worker can proxy requests from `secondary.com` to `yourdomain.com/author{path}` at the CDN edge — no additional server required.
...
**Pros**: Extremely fast (edge); no extra infrastructure; free tier available; the main app sees only `yourdomain.com` requests.
...
**Verdict**: Best production performance, but only works if `secondary.com` DNS is managed through Cloudflare. Worth noting as the recommended production path.
...
**17A is the complete Phase 17 scope.** It solves the stated need — `secondary.com` routes to `/author` — and is fully controllable from the existing backstage UI with minor additions. TLS is not a factor for 17A: the browser simply follows a redirect to the canonical `https://yourdomain.com` domain, where TLS is already handled.
...
1. Add an A record: `secondary.com → [your server IP]` OR CNAME `secondary.com → yourdomain.com`
2. For transparent proxy: your TLS certificate must also cover `secondary.com` (instructions vary by host)
...
```
secondary.com         →  yourdomain.com/author
secondary.com/bio     →  yourdomain.com/author/bio
secondary.com/gallery →  yourdomain.com/author/gallery
```
...
For reference when revisiting — the three approaches analyzed above (Next.js rewrite, separate proxy service, Cloudflare Worker) remain valid options. The Cloudflare Worker approach is likely the cleanest production path if `secondary.com` DNS is managed through Cloudflare. TLS for the secondary domain is the key open question to resolve before implementing 17B — owner to confirm DNS/Cloudflare setup during Phase 19.
