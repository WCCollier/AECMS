...
**FR-004**: 📋 PLANNED (accepted, 2026-06-25) - Registration Controls: (1) `general.default_role` setting replaces hardcoded `'member'` at registration; delete guard becomes dynamic; `'admin'` and `'member'` removed from `RESERVED_NAMES`. (2) `general.require_registration_approval` toggle (Owner-only via `registration.configure` cap); when on, email-verified users are held pending until an admin actions them; approvers (`registration.approve` cap, Owner+Admin default) notified by email; approval panel at `/admin/registrations`; rejection requires reason + soft-deletes account. Toggle-on backfills `approved_at` for all existing verified users. Docs: docs/feature-requests/FR-004-registration-controls.md
...
Current counts: **36 backstage** + **12 customer** = **48 total capabilities** (role.manage added in FR-003). FR-004 will add `registration.configure` + `registration.approve` → **50 total**.
