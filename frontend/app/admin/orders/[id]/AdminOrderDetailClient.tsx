    ...
    <div className="space-y-4">
      ...
      {hasPhysical && order.status === 'processing' && (
        <div className="bg-purple-500/5 border border-purple-500/10 rounded-xl p-4 space-y-3">
          <h3 className="font-semibold text-sm flex items-center gap-2"><Package className="w-4 h-4 text-purple-500" />Ship Physical Items</h3>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs text-foreground/60 mb-1">Carrier</label>
              <input type="text" value={trackingCarrier} onChange={(e) => setTrackingCarrier(e.target.value)} placeholder="UPS, USPS, FedEx…" className="w-full px-3 py-2 text-sm rounded-lg border border-foreground/20 bg-background" />
            </div>
            <div>
              <label className="block text-xs text-foreground/60 mb-1">Tracking Number</label>
              <input type="text" value={trackingNumber} onChange={(e) => setTrackingNumber(e.target.value)} placeholder="1Z999AA1234567890" className="w-full px-3 py-2 text-sm rounded-lg border border-foreground/20 bg-background" />
            </div>
          </div>
          <Button size="sm" onClick={() => patch({ tracking_carrier: trackingCarrier, tracking_number: trackingNumber, mark_shipped: true })} isLoading={isLoading} disabled={!trackingNumber}>
      ...
      {hasPhysical && order.status === 'shipped' && (
        <div className="bg-foreground/5 border border-foreground/10 rounded-xl p-4 space-y-3">
          <h3 className="font-semibold text-sm flex items-center gap-2"><Truck className="w-4 h-4 text-purple-500" />Update Tracking</h3>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs text-foreground/60 mb-1">Carrier</label>
              <input type="text" value={trackingCarrier} onChange={(e) => setTrackingCarrier(e.target.value)} className="w-full px-3 py-2 text-sm rounded-lg border border-foreground/20 bg-background" />
            </div>
            <div>
              <label className="block text-xs text-foreground/60 mb-1">Tracking Number</label>
              <input type="text" value={trackingNumber} onChange={(e) => setTrackingNumber(e.target.value)} className="w-full px-3 py-2 text-sm rounded-lg border border-foreground/20 bg-background" />
            </div>
          </div>
          <div className="flex gap-2">
            <Button size="sm" variant="outline" onClick={() => patch({ tracking_carrier: trackingCarrier, tracking_number: trackingNumber })} isLoading={isLoading}>Save Tracking</Button>
            <Button size="sm" onClick={() => patchStatus('completed')} isLoading={isLoading}><PackageCheck className="w-4 h-4 mr-2" />Mark Delivered</Button>
          </div>
        </div>
      )}
      {hasService && order.status === 'processing' && (
        <div className="bg-indigo-500/5 border border-indigo-500/10 rounded-xl p-4 space-y-3">
          <h3 className="font-semibold text-sm flex items-center gap-2"><CalendarCheck className="w-4 h-4 text-indigo-500" />Schedule Service</h3>
          <div>
            <label className="block text-xs text-foreground/60 mb-1">Appointment Date &amp; Time</label>
            <input type="datetime-local" value={scheduledAt} onChange={(e) => setScheduledAt(e.target.value)} className="w-full px-3 py-2 text-sm rounded-lg border border-foreground/20 bg-background" />
          </div>
          <div>
            <label className="block text-xs text-foreground/60 mb-1">Note to Customer (optional)</label>
            <input type="text" value={scheduledNote} onChange={(e) => setScheduledNote(e.target.value)} placeholder="e.g. Zoom link will be sent separately" className="w-full px-3 py-2 text-sm rounded-lg border border-foreground/20 bg-background" />
          </div>
          <Button size="sm" onClick={() => patch({ mark_scheduled: true, scheduled_at: scheduledAt, scheduled_note: scheduledNote })} isLoading={isLoading}>
            <CalendarCheck className="w-4 h-4 mr-2" />Mark as Scheduled
          </Button>
        </div>
      )}
      {hasService && order.status === 'scheduled' && (
        <div className="bg-green-500/5 border border-green-500/10 rounded-xl p-4 space-y-3">
          ...
          {order.scheduled_at && <p className="text-sm text-foreground/60">Scheduled: {new Date(order.scheduled_at).toLocaleString()}{order.scheduled_note && ` — ${order.scheduled_note}`}</p>}
          <Button size="sm" onClick={() => patchStatus('completed')} isLoading={isLoading}><PackageCheck className="w-4 h-4 mr-2" />Mark Service Complete</Button>
        </div>
      )}
      {hasDigital && (
        <div className="bg-foreground/5 border border-foreground/10 rounded-xl p-4">
          <h3 className="font-semibold text-sm mb-3 flex items-center gap-2"><Package className="w-4 h-4 text-accent" />Digital Items</h3>
      ...
      <div className="flex gap-2 pt-2 border-t border-foreground/10 flex-wrap">
        {order.status === 'processing' && !hasService && !hasPhysical && (
          <Button size="sm" onClick={() => patchStatus('completed')} isLoading={isLoading}><PackageCheck className="w-4 h-4 mr-2" />Mark Completed</Button>
        )}
        {(['pending', 'processing', 'scheduled'] as OrderStatus[]).includes(order.status) && (
          <Button size="sm" variant="outline" onClick={() => patchStatus('cancelled')} isLoading={isLoading}><XCircle className="w-4 h-4 mr-2" />Cancel Order</Button>
        )}
        {(['processing', 'shipped', 'scheduled', 'completed'] as OrderStatus[]).includes(order.status) && (
          <Button size="sm" variant="outline" onClick={() => patchStatus('refunded')} isLoading={isLoading}>Refund</Button>
        )}
      </div>
    </div>
  );
            ...
            <tbody className="divide-y divide-border">
              {(order.items as any[]).map((item) => (
                <tr key={item.id}>
                  ...
                  <td className="px-4 py-3"><span className="text-xs px-2 py-0.5 rounded-full bg-foreground/10 text-foreground/60 capitalize">{item.product?.product_type ?? '—'}</span></td>
      ...
      {shippingAddr && (
        <div>
          ...
          <div className="bg-foreground/5 rounded-xl p-4 text-sm space-y-1">
            ...
            {shippingAddr.city && <p>{shippingAddr.city}{shippingAddr.state ? `, ${shippingAddr.state}` : ''} {shippingAddr.postal_code}</p>}
              ...
              return (
                <li key={entry.id} className="mb-6 ml-4">
                  ...
                  <div className="bg-foreground/5 rounded-lg px-4 py-3 border border-border">
                    <p className="font-medium capitalize">{fromStatus ? `${fromStatus} → ` : ''}{toStatus}</p>
                    <p className="text-xs text-foreground/50 mt-1">{new Date(entry.created_at).toLocaleString()}{entry.user_id ? ` · by ${entry.user_id.slice(0, 8)}…` : ' · system'}</p>
                  </div>
                </li>
              );
            })}
