...
interface MediaPickerProps {
  ...
  onClose?: () => void;
  /** MIME filter, e.g. "image/*". Passed to backend as mime_type param. */
  mimeFilter?: string;
  /** Show pixel dimensions on each grid item. */
  showDimensions?: boolean;
  /** Compact layout: tighter upload zone, 3-col grid, no bottom close row. */
  compact?: boolean;
}
    ...
    <div className="space-y-3">
      ...
      >
        <input
          ref={fileInputRef}
          type="file"
          accept={mimeFilter ? mimeFilter.replace('/*', '/*') : 'image/*'}
        ...
        {isUploading ? (
          ...
          <div className={`flex items-center justify-center gap-2 ${compact ? 'flex-row' : 'flex-col'}`}>
