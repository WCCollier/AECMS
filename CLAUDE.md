...
**FR-001 Deploy 2**: 🚧 NEXT - Unified search bar (single contenteditable input combining tag chips + free text, ALL/ANY slide toggle at left, magnifying-glass/× button at right), ViewModeToggle relocated above grid with result count, SearchResultsEmbed infinite scroll via zone-trailing detection + displayMode override, category DB tables dropped. Full spec in FR-001 doc.
